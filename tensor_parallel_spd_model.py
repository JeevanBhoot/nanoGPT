...
@dataclass
class TensorParallelSPDGPTConfig:
    block_size: int = 1024
...
class TensorParallelCausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: TensorParallelSPDGPTConfig, tp: TensorParallel):
        ...
        cproj_bias = (config.bias and getattr(config, "attn_cproj_bias", False))
...
class TensorParallelSPDGPT(nn.Module):
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        config = TensorParallelSPDGPTConfig(**config_args)
        model = TensorParallelSPDGPT(config)
        sd = model.state_dict()
        sd_keys = sd.keys()
